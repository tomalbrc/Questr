...
public class NavigationComponent implements NavBarComponent {

    @Override
    public MutableComponent build(Context context) {
        int width = context.config().width();

        Vec3 delta = context.bar().getTargetPos().getCenter().subtract(context.player().position());
        ...
        String arrow = getArrow(delta, context.player().getYHeadRot() + 180);
